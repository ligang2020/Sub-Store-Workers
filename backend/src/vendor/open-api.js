...
// Cloudflare's nodejs_compat flag exposes process, so detect Workers first.
const isWorkers = typeof WebSocketPair !== 'undefined';
const isNode =
    eval(`typeof process !== "undefined"`) && !isWorkers; // eval avoids browserify processing
const isStash =
...
export class OpenAPI {
    ...
    // initialize cache
    initCache() {
        // Workers load the persisted cache before accepting a request.  Initializing
        // these defaults also makes the browser-style runtime safe before hydration.
        this.cache = {};
        this.root = {};
        if (isQX)
    ...
    persistCache() {
        ...
        if (isWorkers) {
            const persist = globalThis.__SUB_STORE_WORKERS_PERSIST_CACHE__;
            if (typeof persist === 'function') persist({ cache: this.cache, root: this.root });
        }
        if (isNode) {
            const basePath =
                eval('process.env.SUB_STORE_DATA_BASE_PATH') || '.';
